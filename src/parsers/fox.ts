...
//
// Fox shape: responses are UI "module shells" — a top-level object whose
...
// `parse_fox_list` finds the first list-like thing and flattens it; the
...
/**
 * Common list-bearing keys across Fox module payloads, tried in order.
...
/**
 * Generic parser for any Fox module payload (the DEFAULT for most
 * fox endpoints). Handles:
 *
 ...
 *
 * Fox scoreboard payloads (`ScoreboardResponse` / `LeagueScores`) carry the
 ...
 *
 * Fox standings payloads (`Standings`) wrap rows under
 ...
 *
 * Fox event payloads (`EventData` / `EventMatchup`) are deeply nested module
 ...
 *
 * Fox roster payloads (`TeamRoster`) ship `groups[]`, each a table with
