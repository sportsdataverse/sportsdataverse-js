...
//
// API envelope: every resource responds with `{data: ...}` on success (or
...
// `components.schemas`), so the generic `parse_cbs_list` flattens the most
...
/**
 * Common list-bearing keys in API resource payloads, tried in order.
...
/**
 * Generic parser for any API resource (the DEFAULT for most cbs
 ...
 * row per entry.
 *
 * API standings payloads wrap rows under `data` as a bare array or as
