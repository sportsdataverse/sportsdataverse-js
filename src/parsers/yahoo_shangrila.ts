...
// (graphite-secure.sports.yahoo.com/v1/query/shangrila) flat-API parsers. Each
// parser turns a raw shangrila payload into tidy rectangular rows, same
// contract as src/parsers/cbs_napi.ts / fox_bifrost.ts / mlb_api.ts:
//
...
//
// Shangrila is a GraphQL persisted-query backend: every response is the GraphQL
// envelope `{ data, extensions }`, and `data` carries a SINGLE root field
// (`leagues` / `games` / `players` / `teams` / `olympics` / …) holding the
// rows. `parse_yahoo_shangrila_list` (the default for ~80 endpoints) peels the
...
// route to `parse_yahoo_shangrila_stats`, which unrolls the nested per-player /
...
/**
 * Generic shangrila parser (the DEFAULT for most endpoints). After peeling the
 ...
 */
export function parse_yahoo_shangrila_list(raw: any): Record<string, any>[] {
...
/**
 * Parse a shangrila stats query into one row per player / leader / team stat
 * line.
 *
 ...
 */
export function parse_yahoo_shangrila_stats(raw: any): Record<string, any>[] {
  const data = unwrapData(raw);
  if (!isPlainObject(data)) return [];
 ...
 */
export const YAHOO_SHANGRILA_PARSERS = {
