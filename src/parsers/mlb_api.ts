...
// `sportsdataverse/mlb/mlb_api_parsers.py`:
//
...
//
// Most endpoints use the generic `parse_mlb_api_list` (find the first
 ...
 * the Python `_LIST_KEYS` tuple. `parse_mlb_api_list` walks these and flattens
 ...
 *
 * Use a dedicated parser (`parse_mlb_api_schedule`, `parse_mlb_api_standings`,
 * `parse_mlb_api_person_stats`, …) for endpoints that need extra unrolling.
 */
export function parse_mlb_api_list(raw: any): Record<string, any>[] {
...
/** Parse `mlb_api_teams()` into one row per team (from `teams[]`). */
export function parse_mlb_api_teams(raw: any): Record<string, any>[] {
  return normalize(raw?.teams ?? []);
}

/**
 * Parse `mlb_api_schedule()` into one row per game.
 *
 ...
 */
export function parse_mlb_api_schedule(raw: any): Record<string, any>[] {
...
/**
 * Parse `mlb_api_team_roster()` into one row per player (from `roster[]`).
 *
 ...
 */
export function parse_mlb_api_team_roster(raw: any): Record<string, any>[] {
 ...
 */
export function parse_mlb_api_standings(raw: any): Record<string, any>[] {
  if (!isPlainObject(raw)) return [];
...
/**
 * Parse `mlb_api_person_stats()` / `mlb_api_team_stats()` into one row per
 * stats split.
 *
 ...
 */
export function parse_mlb_api_person_stats(raw: any): Record<string, any>[] {
  if (!isPlainObject(raw)) return [];
 ...
 */
export function parse_mlb_api_boxscore(raw: any): Record<string, any>[] {
  if (!isPlainObject(raw)) return [];
...
/**
 * Parse `mlb_api_linescore()` into one row per inning (from `innings[]`).
 *
 * Input: `{innings: [{num, ordinalNum, home: {runs, hits, errors}, away:
 * {...}}, ...]}`. Each inning flattens so home/away become `home_runs` /
 * `away_hits` columns.
 */
export function parse_mlb_api_linescore(raw: any): Record<string, any>[] {
  if (!isPlainObject(raw)) return [];
  return normalize(raw.innings ?? []);
}

/**
 * Parse `mlb_api_play_by_play()` into one row per play (from `allPlays[]`).
 *
 ...
 */
export function parse_mlb_api_play_by_play(raw: any): Record<string, any>[] {
  if (!isPlainObject(raw)) return [];
 ...
 * directly.
 */
export function parse_mlb_api_win_probability(raw: any): Record<string, any>[] {
 ...
 * `pick_*` columns.
 */
export function parse_mlb_api_draft_latest(raw: any): Record<string, any>[] {
 ...
 */
export function parse_mlb_api_timecodes(raw: any): Record<string, any>[] {
 ...
 * `MLB_API_ENDPOINT_PARSERS` registry but is keyed by the *parser function
 * name* the YAML references (the JS generator threads `def.parser` straight
 * through `parserFor`). Endpoints whose `parser:` is `parse_mlb_api_list` are
 * not listed individually — they resolve to the generic flattener by name.
 */
export const MLB_API_PARSERS = {
  parse_mlb_api_list,
  parse_mlb_api_teams,
