// Yahoo Sports Editorial API (api-secure.sports.yahoo.com/v1/scores/s)
// flat-API parsers. Each parser turns a raw scores payload into tidy
// rectangular rows, same contract as src/parsers/cbs.ts /
// fox.ts / mlb.ts:
//
...
// column). The generic `parse_yahoo_scores_list` is the catch-all that
...
/**
 * Common list-bearing keys in scores payloads, tried in order. `firstListIn`
...
/**
 * Peel the scores `{service: ...}` envelope once. Returns the inner
...
/**
 * Generic scores parser (catch-all). After peeling the `service` envelope it
...
/**
 * Parse the scores scoreboard into one row per game.
 *
...
/**
 * Parse the scores boxscore into one row per player.
 *
