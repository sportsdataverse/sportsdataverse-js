...
describe('parsers/fox: parse_fox_list (generic flattener)', () => {
...
describe('parsers/fox: parse_fox_scoreboard', () => {
...
describe('parsers/fox: parse_fox_standings', () => {
  it('unrolls standingsSections[].standings[] into one row per entry w/ section meta', () => {
    ...
    const rows = parse_fox_standings(raw);
    rows.length.should.equal(2);
  ...
  it('falls back to the generic flattener for un-sectioned payloads', () => {
    const rows = parse_fox_standings({ groupList: [{ id: 'x', title: 'X' }] });
...
describe('parsers/fox: parse_fox_event', () => {
  ...
  it('falls back to gameStats.items[] then to a single shell row', () => {
    parse_fox_event({ gameStats: { items: [{ title: 'A' }] } })[0].should.have.property(
      'title',
      'A'
    );
...
describe('parsers/fox: parse_fox_team_roster', () => {
