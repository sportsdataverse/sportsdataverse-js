...
} from '../../dist/parsers/yahoo_shangrila.js';
...
describe('parsers/yahoo_shangrila: parse_yahoo_shangrila_list (generic)', () => {
  ...
  it('emits a single root object with no inner list as one row', () => {
    const rows = parse_yahoo_shangrila_list({ data: { leagueId: 'nfl', name: 'NFL' } });
    rows.length.should.equal(1);
    rows[0].should.have.property('league_id', 'nfl');
  });

  it('returns [] for empty / malformed payloads', () => {
    parse_yahoo_shangrila_list({ data: {} }).should.eql([]);
    parse_yahoo_shangrila_list(null).should.eql([]);
    parse_yahoo_shangrila_list('nope').should.eql([]);
  });
});

describe('parsers/yahoo_shangrila: parse_yahoo_shangrila_stats', () => {
  it('unrolls leagues[].footballStats[] into one row per player w/ league meta', () => {
    ...
    const rows = parse_yahoo_shangrila_stats(raw);
    rows.length.should.equal(2);
  ...
  it('returns [] for empty / malformed payloads', () => {
    parse_yahoo_shangrila_stats({ data: {} }).should.eql([]);
    parse_yahoo_shangrila_stats(null).should.eql([]);
  });
});

describe('parsers/yahoo_shangrila: registry wiring', () => {
  it('registers both yahoo_shangrila parsers by name', () => {
    for (const name of ['parse_yahoo_shangrila_list', 'parse_yahoo_shangrila_stats']) {
...
describe('yahoo_shangrila flat-API family metadata (flat-contract style)', () => {
  const family = () => FLAT_WRAPPERS.filter((w) => w.api === 'yahoo_shangrila');

  it('registers the yahoo_shangrila family (105 endpoints) on https://graphite-secure.sports.yahoo.com', () => {
    const rows = family();
    rows.length.should.equal(105);
    FLAT_HOSTS.yahoo_shangrila.should.equal('https://graphite-secure.sports.yahoo.com');
    for (const w of rows) w.host.should.equal('https://graphite-secure.sports.yahoo.com');
  });

  it('every yahoo_shangrila wrapper names a registered parser, none auth', () => {
    for (const w of family()) {
      (typeof w.parser).should.equal('string', `parser missing on ${w.short}`);
      w.parser.should.startWith('parse_yahoo_shangrila_');
      (typeof parserFor(w.parser)).should.equal('function', `parser ${w.parser} not registered`);
      should(w.auth).not.be.true(`unexpected auth flag on yahoo_shangrila_${w.short}`);
    }
  });

  it('uses the generic list parser as the default for most endpoints', () => {
    const rows = family();
    const generic = rows.filter((w) => w.parser === 'parse_yahoo_shangrila_list').length;
    generic.should.be.above(rows.length / 2);
  });

  it('routes the stats queries to the dedicated stats parser', () => {
    const stats = family().filter((w) => w.parser === 'parse_yahoo_shangrila_stats');
...
describe('yahoo namespace (both stems share sdv.yahoo)', () => {
  it('creates the standalone sdv.yahoo namespace with both stems (snake + camel)', () => {
    ...
    for (const snake of [
      ...
      'yahoo_shangrila_league_standings',
      'yahoo_shangrila_league_stats_individual',
    ]) {
