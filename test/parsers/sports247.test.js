...
describe('parsers/sports247: parse_sports247_list (generic flattener)', () => {
  it('flattens a bare top-level JSON array (the common RDB shape)', () => {
    ...
    const rows = parse_sports247_list(raw);
  ...
  it('walks the LIST_KEYS envelope (list / rankings / items / …)', () => {
    parse_sports247_list({ list: [{ teamId: 7, name: 'A' }] })[0].should.have.property(
      'team_id',
      7
    );
    parse_sports247_list({ rankings: [{ key: 9 }] })[0].should.have.property('key', 9);
    parse_sports247_list({ items: [{ id: 'x' }] })[0].should.have.property('id', 'x');
  });

  it('deep-flattens nested objects and stringifies array cells', () => {
    const rows = parse_sports247_list([
  ...
  it('returns [] for empty / malformed payloads', () => {
    parse_sports247_list([]).should.eql([]);
    parse_sports247_list({}).should.eql([]);
    parse_sports247_list({ list: [] }).should.eql([]);
    parse_sports247_list(null).should.eql([]);
    parse_sports247_list('nope').should.eql([]);
  });
});

describe('parsers/sports247: parse_sports247_paged_list', () => {
  it('flattens the `list[]` of a {pagination, list} paged envelope', () => {
    ...
    const rows = parse_sports247_paged_list(raw);
    rows.length.should.equal(2);
  ...
  it('also accepts a bare array, and returns [] when no list', () => {
    parse_sports247_paged_list([{ key: 1 }])[0].should.have.property('key', 1);
    parse_sports247_paged_list({}).should.eql([]);
    parse_sports247_paged_list(null).should.eql([]);
  });
});

describe('parsers/sports247: parse_sports247_institution_rankings', () => {
  it('unrolls list[] and prefixes the pagination_* context onto each row', () => {
    ...
    const rows = parse_sports247_institution_rankings(raw);
    rows.length.should.equal(2);
  ...
  it('handles a bare array and empty payloads', () => {
    parse_sports247_institution_rankings([{ name: 'X', rank: 5 }])[0].should.have.property(
      'rank',
      5
    );
    parse_sports247_institution_rankings({ list: [] }).should.eql([]);
    parse_sports247_institution_rankings({}).should.eql([]);
    parse_sports247_institution_rankings(null).should.eql([]);
  });
});

describe('parsers/sports247: parse_sports247_ranking_feed', () => {
  it('flattens the `rankings[]` of a PlayerRankingFeedDto envelope', () => {
    ...
    const rows = parse_sports247_ranking_feed(raw);
    rows.length.should.equal(2);
  ...
  it('returns [] for an empty / missing rankings block', () => {
    parse_sports247_ranking_feed({ rankings: [] }).should.eql([]);
    parse_sports247_ranking_feed({}).should.eql([]);
    parse_sports247_ranking_feed(null).should.eql([]);
  });
});

describe('parsers/sports247: registry wiring', () => {
  it('registers all four sports247 parsers by name', () => {
    for (const name of [
      'parse_sports247_list',
      'parse_sports247_paged_list',
      'parse_sports247_institution_rankings',
...
describe('sports247 flat-API family metadata (flat-contract style)', () => {
  const family = () => FLAT_WRAPPERS.filter((w) => w.api === 'sports247');

  it('registers the sports247 family (25 endpoints) on https://api.247sports.com', () => {
    const rows = family();
    rows.length.should.equal(25);
    FLAT_HOSTS.sports247.should.equal('https://api.247sports.com');
    for (const w of rows) w.host.should.equal('https://api.247sports.com');
  });

  it('every sports247 wrapper names a registered parser, none auth', () => {
    for (const w of family()) {
      (typeof w.parser).should.equal('string', `parser missing on ${w.short}`);
      w.parser.should.startWith('parse_sports247_');
      (typeof parserFor(w.parser)).should.equal('function', `parser ${w.parser} not registered`);
      should(w.auth).not.be.true(`unexpected auth flag on sports247_${w.short}`);
    }
  });

  it('uses the generic list parser as the default for most endpoints', () => {
    const rows = family();
    const generic = rows.filter((w) => w.parser === 'parse_sports247_list').length;
