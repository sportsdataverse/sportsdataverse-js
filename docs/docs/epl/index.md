...
`sdv.epl` is composed from **written, documented source** (`src/generated/espn/epl.ts`) — a phased proof of converting the runtime wrapper factory into reviewable modules. Every endpoint is a real `export const` with JSDoc, callable as `sdv.epl.espnEpl<Endpoint>(params)` and under its snake_case alias (`espn_epl_<endpoint>`) for parity with the Python / R packages.
