...
`sdv.wc` is composed from **written, documented source** (`src/generated/espn/wc.ts`) — a phased proof of converting the runtime wrapper factory into reviewable modules. Every endpoint is a real `export const` with JSDoc, callable as `sdv.wc.espnWc<Endpoint>(params)` and under its snake_case alias (`espn_wc_<endpoint>`) for parity with the Python / R packages.
