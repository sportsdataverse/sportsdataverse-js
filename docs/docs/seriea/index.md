...
`sdv.seriea` is composed from **written, documented source** (`src/generated/espn/seriea.ts`) — a phased proof of converting the runtime wrapper factory into reviewable modules. Every endpoint is a real `export const` with JSDoc, callable as `sdv.seriea.espnSeriea<Endpoint>(params)` and under its snake_case alias (`espn_seriea_<endpoint>`) for parity with the Python / R packages.
