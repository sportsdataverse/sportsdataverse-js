...
# `sdv.nfl` — Native (non-ESPN) APIs

Beyond the ESPN surface, `sdv.nfl` also wraps the league's own live APIs. Same `{ parsed: true }` contract; each method is exposed under both snake_case and camelCase on `sdv.nfl`.
